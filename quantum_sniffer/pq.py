...
def classify_tls_group(group_id):
    """Classify a TLS named-group ID. Unknown -> None."""
    return TLS_GROUP_CLASS.get(group_id)


def classify_ike_dh(transform_id):
...
def classify_connection(info):
    """Determine PQ status of a connection from analyzer-emitted fields.

    Returns one of: "Yes", "Hybrid", "No", "Unknown".
    """
    saw_pq = False
    saw_hybrid = False
    saw_classical = False
    ...
    # Protocols whose crypto is fixed and known
